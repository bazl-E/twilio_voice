...
class IncomingCallActivity : AppCompatActivity() {
    ...
    companion object {
        ...
        /**
         * Static field to hold call data when the user answers a call from the lock screen.
         * When the device is locked, we answer the call (audio connects) but do NOT launch
         * MainActivity — instead the user sees the lock screen. After the user unlocks,
         * MainActivity.onResume() checks this field and navigates to the active call screen.
         * The field is cleared after being consumed.
         */
        @Volatile
        @JvmStatic
    ...
    private fun handleAnswerFromNotification() {
        ...
        if (sid != null) {
            ...
            if (deviceLocked) {
                ...
                android.util.Log.d(TAG, "handleAnswerFromNotification: Device is unlocked - launching main Flutter activity")
                launchMainActivity()
            }
        } else {
    ...
    private fun showOverLockScreen() {
        ...
        // NOTE: FLAG_DISMISS_KEYGUARD is intentionally NOT included here.
        // It triggers a system unlock dialog on secure lock screens (PIN/pattern/fingerprint)
        // that steals focus and blocks ALL touch events on the activity — the user can see
        // the incoming call UI but cannot interact with answer/decline buttons.
        // Instead, we use FLAG_SHOW_WHEN_LOCKED to show the activity OVER the lock screen
        // and only dismiss the keyguard when the user answers (in proceedWithAnswer()).
        @Suppress("DEPRECATION")
        window.addFlags(
        ...
        // STEP 4: Keyguard dismissal is NOT done here.
        // requestDismissKeyguard() triggers a system unlock prompt on secure lock screens
        // (PIN/pattern/fingerprint) that blocks all touch events on the incoming call UI.
        // The keyguard is only dismissed when the user answers the call (see proceedWithAnswer()).
        // setShowWhenLocked(true) + FLAG_SHOW_WHEN_LOCKED already make the activity
        // visible and interactive over the lock screen.
    ...
    private fun bringActivityToFront() {
        try {
            ...
            // NOTE: requestDismissKeyguard() was intentionally removed here.
            // It triggers a system unlock dialog on secure lock screens that blocks
            // all touch events on the incoming call UI. Keyguard is only dismissed
            // when the user answers the call (see proceedWithAnswer()).
    ...
    override fun onAttachedToWindow() {
        ...
        // NOTE: FLAG_DISMISS_KEYGUARD intentionally omitted — it blocks touch events
        @Suppress("DEPRECATION")
        window.addFlags(
            WindowManager.LayoutParams.FLAG_SHOW_WHEN_LOCKED or
            WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON or
    ...
    private fun proceedWithAnswer() {
        ...
        callSid?.let { sid ->
            ...
            val deviceLocked = isDeviceLocked()
            android.util.Log.d(TAG, "proceedWithAnswer: isDeviceLocked=$deviceLocked")
            ...
            if (deviceLocked) {
                ...
                android.util.Log.d(TAG, "proceedWithAnswer: Device is unlocked - launching main Flutter activity")
                launchMainActivity()
            }
        } ?: run {
    ...
    /**
     * Store pending call data in a static field so that MainActivity can retrieve it
     * when the user unlocks the device and the activity resumes.
     */
    private fun storePendingCallDataForMainActivity() {
        ...
        android.util.Log.d(TAG, "storePendingCallDataForMainActivity: Stored data=$pendingAnsweredCallData")
    }
